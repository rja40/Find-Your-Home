...
object RTenant {
  ...
  val feeder = csv("tenant.csv").eager.random

  val rtenant = forever("i") {
    feed(feeder)
    .exec(http("RTenant ${i}")
      .get("/api/v1/tenant/${UUID}"))
      .pause(1)
  }
...
object RTenantVarying {
  val feeder = csv("tenant.csv").eager.circular

  val rtenant = forever("i") {
    feed(feeder)
    .exec(http("RTenantVarying ${i}")
      .get("/api/v1/tenant/${UUID}"))
    .pause(1, 60)
  }
}
...
/*
  Failed attempt to interleave reads from User and Tenant tables.
  The Gatling EDSL only honours the second (Tenant) read,
  ignoring the first read of User. [Shrug-emoji] 
 */
object RBoth {

  val u_feeder = csv("users.csv").eager.circular
  val m_feeder = csv("tenant.csv").eager.random
...
class ReadTenantSim extends ReadTablesSim {
  val scnReadTenant = scenario("ReadTenant")
    .exec(RTenant.rtenant)

  setUp(
    scnReadTenant.inject(atOnceUsers(Utility.envVarToInt("USERS", 1)))
  ).protocols(httpProtocol)
}
...
class ReadBothVaryingSim extends ReadTablesSim {
  val scnReadMV = scenario("ReadTenantVarying")
    .exec(RTenantVarying.rtenant)
...
/*
  This doesn't work---it just reads the Tenant table.
