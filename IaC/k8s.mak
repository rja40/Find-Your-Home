...
DOCKER_REG=anuj-saboo
AWS_REGION=us-east-1
JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk-amd64
GAT_DIR=/home/anuj/softwares/gatling-charts-highcharts-bundle-3.5.0-bundle/gatling-charts-highcharts-bundle-3.5.0/bin
...
rollout-landlord: landlord
	$(KC) rollout -n $(APP_NS) restart deployment/service-landlord

# --- rollout-landlord: Rollout a new deployment of property
...
#
dynamodb-clean:
	$(AWS) cloudformation delete-stack --stack-name db-anuj-saboo || true | tee $(LOG_DIR)/dynamodb-clean.log
...
$(LOG_DIR)/dynamodb-init.log: cluster/cloudformationdynamodb.json
	...
	$(AWS) cloudformation create-stack --stack-name db-anuj-saboo --template-body file://$< || true | tee $(LOG_DIR)/dynamodb-init.log

# Update S1 and associated monitoring, rebuilding if necessary
landlord: $(LOG_DIR)/landlord.repo.log cluster/service-landlord.yaml cluster/service-landlord-sm.yaml cluster/service-landlord-vs.yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord.yaml | tee $(LOG_DIR)/landlord.log
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-sm.yaml | tee -a $(LOG_DIR)/landlord.log
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-vs.yaml | tee -a $(LOG_DIR)/landlord.log
...
cri: $(LOG_DIR)/landlord.repo.log $(LOG_DIR)/property.repo.log $(LOG_DIR)/tenant.repo.log $(LOG_DIR)/db.repo.log

# Build the landlord service
$(LOG_DIR)/landlord.repo.log: ../code/service-landlord/Dockerfile ../code/service-landlord/app.py ../code/service-landlord/requirements.txt
	$(DK) build -t $(CREG)/$(REGID)/team-a-service-landlord:$(APP_VER_TAG) ../code/service-landlord | tee $(LOG_DIR)/landlord.img.log
	$(DK) push $(CREG)/$(REGID)/team-a-service-landlord:$(APP_VER_TAG) | tee $(LOG_DIR)/landlord.repo.log
